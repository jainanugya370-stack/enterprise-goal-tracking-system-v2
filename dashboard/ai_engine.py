...
# ✅ Use Gemini in production (Render) — Ollama only works locally
...
def generate_executive_ai_summary():
    ...
    avg = updates.aggregate(Avg('achievement_value'))['achievement_value__avg'] or 0
    ...
    prompt = f"""
Organization:
- Employees: {total_employees}, Goals: {total_goals}, Approved: {approved_goals}
- Avg Achievement: {round(avg, 2)}
...
"""
    ...
    return {
        ...
        "total_goals":        total_goals,
        "approved_goals":     approved_goals,
        "average_achievement": round(avg, 2),
